...
const REQUIRED = ["entry", "stop", "exit_price", "gross_pnl"]
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-6 mb-6">
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-3">
        ...
        <Field label="入场价 *" required={error && !form.entry}><Input value={form.entry} onChange={uf("entry")} placeholder="1.03250" /></Field>
        <Field label="止损价 *" required={error && !form.stop}><Input value={form.stop} onChange={uf("stop")} placeholder="1.02950" /></Field>
        <Field label="目标价"><Input value={form.target} onChange={uf("target")} placeholder="选填" /></Field>
        <Field label="出场价 *" required={error && !form.exit_price}><Input value={form.exit_price} onChange={uf("exit_price")} placeholder="1.03720" /></Field>
        <Field label="盈亏 (USD) *" required={error && !form.gross_pnl}><Input value={form.gross_pnl} onChange={uf("gross_pnl")} placeholder="235" /></Field>
