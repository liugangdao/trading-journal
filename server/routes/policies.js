...
router.post('/', (req, res) => {
  try {
    ...
    const result = db.prepare(
      'INSERT INTO policies (category, title, content, sort_order) VALUES (?, ?, ?, ?)'
    ).run(category, title.trim(), content.trim(), sort_order ?? 0)
...
router.put('/:id', (req, res) => {
  try {
    ...
    const { category, title, content, sort_order } = req.body
    db.prepare(
      "UPDATE policies SET category = ?, title = ?, content = ?, sort_order = ?, updated_at = datetime('now') WHERE id = ?"
    ).run(
...
router.put('/:id/toggle', (req, res) => {
  try {
    ...
    db.prepare("UPDATE policies SET is_active = ?, updated_at = datetime('now') WHERE id = ?").run(newActive, req.params.id)
    const policy = db.prepare('SELECT * FROM policies WHERE id = ?').get(req.params.id)
    res.json(policy)
  } catch (err) {
    res.status(500).json({ error: err.message })
